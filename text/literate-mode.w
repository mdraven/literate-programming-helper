...
parse-file возвращает таблицу с информацией по чанкам. Но нам нужен сгенерированый
...
В конце удаляем пробелы перед первой строкой, тогда она будет выравнена по начальной
  позиции тега.
Так как insert-parts-of-chunks помещает новые оверлеи на вершину списка в переменной
  *overlays*, то нижележащие слои будут идти в списке после вышележащих.
...
том где удалить пробелы, а где добавить. Из *overlays* вычитаются найденные
